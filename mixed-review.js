...
function sumUpTo(num) {
  let sum = 0;
  for (let addend = 1; addend <= num; addend++) {
    sum += addend;
  }
  return sum;
}
...
// If your function is correct, these should both print true.
// console.log(totalLetters(["a", "bc", "defg"]) == 7); console.log(totalLetters(["abcdefghij"]) == 10);
...
//     let number = arr[i];
//     if (numberFrequency[number] == undefined) {
...
//   }
//
//   let frequencyOfNum = numberFrequency[num];
//   let counter = 0;
//   for (let j = 0; j < arr.length; j++) {
//     if (arr[j] == num) {
//       counter++;
//       if (counter == frequencyOfNum) {
//         return j;
//       }
//     }
//   }
...
function lastTime(arr, num) {
  for (let numberIndex = arr.length - 1; numberIndex >= 0; numberIndex--) {
    if (arr[numberIndex] == num) {
      return numberIndex;
    }
  }
...
function startAndEndWith(letter) {
  ...
  for (let i = 0; i < words.length; i++) {
    let word = words[i];
      if (word[0] == letter.toLowerCase() && word[word.length - 1] == letter.toLowerCase()) {
        wordArray.push(word);
      }
    }
  return wordArray;
...
function countMoney(coins) {
  let sum = 0;
  let coinType = Object.keys(coins);
  let coinAmount = Object.values(coins);

  if (coins.hasOwnProperty("quarters")) {
      sum += 25 * coins["quarters"];
  }
  if (coins.hasOwnProperty("dimes")) {
      sum += 10 * coins["dimes"];
  }
  if (coins.hasOwnProperty("nickels")) {
      sum += 5 * coins["nickels"];
  }
...
function waitFor24() {
  let userNum;
  while (userNum != 24) {
    userNum = parseInt(readline.question("Please type a number: "));
  }
}
