...
let highestMultipler = parseInt(readline.question("Enter the highest integer multipler you want this multiplication table to go up to: "));
...
let table = [];
for (let rowNum = 1; rowNum <= highestMultipler; rowNum++) {
  let row = [];
  for (let colNum = 1; colNum <= highestMultipler; colNum++) {
...
for (let arrNum = 0; arrNum < highestMultipler; arrNum++) {
  console.log(table[arrNum]);
}
...
for (let factorIndex = 0; factorIndex < highestMultipler; factorIndex++) {
  str = "";
  for (let multiplerIndex = 0; multiplerIndex < highestMultipler; multiplerIndex++) {
    if (table[factorIndex][multiplerIndex].toString().length == 1) {
      str += "  " + table[factorIndex][multiplerIndex];
    } else {
      str += " " + table[factorIndex][multiplerIndex];
    }
  }
