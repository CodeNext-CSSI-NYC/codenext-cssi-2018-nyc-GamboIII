...
let wordsWith12Letters = [];
for (let i = 0; i < words.length; i++) {
  if (words[i].length == 12) {
    wordsWith12Letters.push(words[i]);
  }
}

//console.log(wordsWith12Letters);
...
for (let i = 0; i < words.length; i++) {
  if (words[i].includes("x")) {
    if (words[i].includes("z")) {
      wordsWithXandZ.push(words[i]);
    }
  }
}

//console.log(wordsWithXandZ);
...
//console.log(letterCountAgain("Super Smash Bros. Melee for the Nintendo Gamecube"));
